...
public static class PdfDomain
{
    ...
    public sealed record ParsedPdfOptions(
        PocketCalculator.Browser.RasterPdfOptions Raster,
    ...
    public static List<PocketCalculator.Browser.RasterPdfPageRange> ParsePageRanges(string value)
    {
        ...
        var ranges = new List<PocketCalculator.Browser.RasterPdfPageRange>();
        ...
        for (int index = 0; index < parts.Length; index++)
        {
            ...
            ranges.Add(new PocketCalculator.Browser.RasterPdfPageRange(start, end));
        }
        ...
        List<PocketCalculator.Browser.RasterPdfPageRange> sorted =
            [.. ranges.OrderBy(range => range.Start ?? 1).ThenBy(range => range.End ?? int.MaxValue)];
        var normalized = new List<PocketCalculator.Browser.RasterPdfPageRange>();
        foreach (PocketCalculator.Browser.RasterPdfPageRange range in sorted)
        {
            int start = range.Start ?? 1;
            if (normalized.Count > 0)
            {
                PocketCalculator.Browser.RasterPdfPageRange previous = normalized[^1];
            ...
            normalized.Add(new PocketCalculator.Browser.RasterPdfPageRange(start, range.End));
        }
    ...
    public static ParsedPdfOptions ParseOptions(JsonNode? parameters)
    {
        ...
        List<PocketCalculator.Browser.RasterPdfPageRange> pageRanges = [];
        ...
        var defaults = new PocketCalculator.Browser.RasterPdfOptions();
        var raster = new PocketCalculator.Browser.RasterPdfOptions
        {
        ...
        string? sessionId)
    {
        ...
        catch (PocketCalculator.Browser.RasterPdfException error)
        {
