...
using TaffyAlignContentKeyword = PocketCalculator.Render.Layout.AlignContentKeyword;
using TaffyAlignItemsKeyword = PocketCalculator.Render.Layout.AlignItemsKeyword;
using TaffyFlexDirection = PocketCalculator.Render.Layout.FlexDirection;
using TaffyFlexWrap = PocketCalculator.Render.Layout.FlexWrap;
using TaffyGridAutoFlow = PocketCalculator.Render.Layout.GridAutoFlow;
...
public sealed partial class PreparedRender
{
    ...
    public Dictionary<string, string>? ComputedStyle(NodeId id, string? pseudoElement)
    {
        ...
        {
            PocketCalculator.Render.Float.Left => "left",
            PocketCalculator.Render.Float.Right => "right",
            _ => "none",
        };
        output["clear"] = style.Clear switch
        {
            PocketCalculator.Render.Clear.Left => "left",
            PocketCalculator.Render.Clear.Right => "right",
            PocketCalculator.Render.Clear.Both => "both",
            _ => "none",
        };
        ...
        output["white-space"] = (style.WhiteSpace ?? PocketCalculator.Render.WhiteSpace.Normal) switch
        {
            PocketCalculator.Render.WhiteSpace.Normal => "normal",
            PocketCalculator.Render.WhiteSpace.NoWrap => "nowrap",
            PocketCalculator.Render.WhiteSpace.Pre => "pre",
            PocketCalculator.Render.WhiteSpace.PreWrap => "pre-wrap",
            PocketCalculator.Render.WhiteSpace.PreLine => "pre-line",
            _ => "break-spaces",
        };
        ...
        string overflowWrap = (style.OverflowWrap ?? PocketCalculator.Render.OverflowWrap.Normal) switch
        {
            PocketCalculator.Render.OverflowWrap.Normal => "normal",
            PocketCalculator.Render.OverflowWrap.BreakWord => "break-word",
            _ => "anywhere",
        };
        ...
        output["word-break"] = (style.WordBreak ?? PocketCalculator.Render.WordBreak.Normal) switch
        {
            PocketCalculator.Render.WordBreak.Normal => "normal",
            PocketCalculator.Render.WordBreak.BreakAll => "break-all",
