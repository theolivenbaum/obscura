...
internal static partial class PageHelpers
{
    ...
    /// <summary>
    /// Parse <c>POCKETCALCULATOR_GEOLOCATION="lat,lon"</c> for the navigator.geolocation shim.
    /// </summary>
    /// <remarks>
    ...
    {
        string? raw = Environment.GetEnvironmentVariable("POCKETCALCULATOR_GEOLOCATION");
        if (raw is null)
        {
            return null;
        }
    ...
    internal static TimeSpan DefaultNavigationTimeout() =>
        NavigationTimeoutFromEnvValue(Environment.GetEnvironmentVariable("POCKETCALCULATOR_NAV_TIMEOUT_MS"));
    ...
    internal static int DefaultNavigationChainLimitValue() =>
        NavigationChainLimitFromEnvValue(Environment.GetEnvironmentVariable("POCKETCALCULATOR_NAV_CHAIN_LIMIT"));

    /// <summary>
    /// Only an unreadable value falls back to the default, so
    /// <c>POCKETCALCULATOR_NAV_CHAIN_LIMIT=0</c> and <c>SetNavigationChainLimit(0)</c> agree.
    /// </summary>
    ...
    internal static int MaxLiveFrames() =>
        Environment.GetEnvironmentVariable("POCKETCALCULATOR_MAX_LIVE_FRAMES") is { } value
    ...
    internal static int ResponseBodyEntryLimit() =>
        Environment.GetEnvironmentVariable("POCKETCALCULATOR_NETWORK_BODY_BUFFER_ENTRIES") is { } value
        && ulong.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out ulong parsed)
            ? (int)Math.Min(parsed, int.MaxValue)
            : 128;

    internal static int ResponseBodyByteLimit() =>
        Environment.GetEnvironmentVariable("POCKETCALCULATOR_NETWORK_BODY_BUFFER_BYTES") is { } value
        && ulong.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out ulong parsed)
            ? (int)Math.Min(parsed, int.MaxValue)
            : 2 * 1024 * 1024;
    ...
    /// <c>POCKETCALCULATOR_DOCUMENT_GC_THRESHOLD_MB=0</c> disables the release entirely.
    /// </para>
    /// </remarks>
    internal static void ReleaseReplacedDocumentMemory()
    {
        ulong thresholdMb = EnvUlong(
            "POCKETCALCULATOR_DOCUMENT_GC_THRESHOLD_MB", DefaultDocumentGcThresholdMb);
        if (thresholdMb == 0)
        {
            return;
        }
