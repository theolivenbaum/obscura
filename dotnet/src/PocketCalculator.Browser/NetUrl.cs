...
/// <see cref="Uri"/> <c>PocketCalculator.Net</c> still speaks.
/// </summary>
/// <remarks>
...
/// today either. Migrating <c>PocketCalculator.Net</c> to <see cref="UrlRecord"/> removes the
...
internal static class NetUrl
{
    ...
    internal static Uri From(UrlRecord url) =>
        Uri.TryCreate(url.Href, UriKind.Absolute, out Uri? uri)
            ? uri
            : throw PocketCalculatorNetException.Network($"{url.Href}: error sending request for url ({url.Href})");
