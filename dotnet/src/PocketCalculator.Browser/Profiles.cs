...
public static class Profiles
{
    ...
    /// <c>POCKETCALCULATOR_PROFILE=&lt;index&gt;</c> pins a specific profile,
    /// <c>POCKETCALCULATOR_ROTATE_PROFILE=1</c> picks a random profile per context.
    /// </remarks>
    public static BrowserProfile SelectProfile()
    {
        string? raw = Environment.GetEnvironmentVariable("POCKETCALCULATOR_PROFILE")?.Trim();
        if (raw is not null
        ...
        return EnvEnabled("POCKETCALCULATOR_ROTATE_PROFILE") ? RandomProfile() : All[0];
    }
