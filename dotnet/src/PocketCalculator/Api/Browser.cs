...
using InnerPage = PocketCalculator.Browser.Page;
...
/// and throws <see cref="PocketCalculatorException"/> if construction ever does fail.
/// </para>
/// <para>
...
/// <c>PocketCalculator.Browser</c> and <c>PocketCalculator.Net</c>, which this assembly references
...
public sealed class Browser
{
    ...
    private static void ApplyFontDirectories(IList<string>? directories)
    {
        ...
        foreach (string directory in directories)
        {
            ...
            {
                throw new PocketCalculatorException(
                    PocketCalculatorErrorKind.Internal,
                    $"Font directory does not exist or is not a directory: {directory}");
            }
        }

        string[] requested = [.. directories];
        if (!PocketCalculator.Render.FontDirectories.Configure(requested)
            && !(PocketCalculator.Render.FontDirectories.Configured is { } current
