...
// set POCKETCALCULATOR_TIMEZONE to match the exit IP's region. An existing TZ from the
...
var configuredTimezone = Environment.GetEnvironmentVariable("POCKETCALCULATOR_TIMEZONE");
...
// The js-side fetch path (op_fetch_url) reads POCKETCALCULATOR_ALLOW_PRIVATE_NETWORK
...
{
    ProcessEnvironment.Set("POCKETCALCULATOR_ALLOW_PRIVATE_NETWORK", "1");
}
...
static void InstallVersionAction(RootCommand root)
{
    ...
    // "obscura <version>" from the build script's POCKETCALCULATOR_BUILD_VERSION.
