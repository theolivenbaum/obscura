...
public static class WorkerHost
{
    ...
    {
        if (Environment.GetEnvironmentVariable("POCKETCALCULATOR_WORKER") == "1")
        {
            return true;
        }
    ...
    {
        var proxy = Environment.GetEnvironmentVariable("POCKETCALCULATOR_PROXY")?.Trim();
        if (proxy is { Length: 0 })
        {
            proxy = null;
        }
        var stealth = EnvFlag(Environment.GetEnvironmentVariable("POCKETCALCULATOR_STEALTH"));
        var obeyRobots = EnvFlag(Environment.GetEnvironmentVariable("POCKETCALCULATOR_OBEY_ROBOTS"));
