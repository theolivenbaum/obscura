...
using PocketCalculator.Cli.Json;
...
public static class FetchCommand
{
        ...
        string? screenshot)
    {
        ...
        (float Width, float Height)? screenshotViewport = screenshot is null
            ? null
            : (EnvFloat("POCKETCALCULATOR_SHOT_W") ?? 1280.0f, EnvFloat("POCKETCALCULATOR_SHOT_H") ?? 720.0f);
        ...
        var evalAtCaptureBoundary = screenshot is not null
            && eval is not null
            && Environment.GetEnvironmentVariable("POCKETCALCULATOR_SHOT_EVAL_AT_CAPTURE") == "1";
    ...
    private static async Task CaptureScreenshotAsync(
        ...
        bool quiet)
    {
        var resourceDeadlineMs = EnvUlong("POCKETCALCULATOR_RENDER_RESOURCE_DEADLINE_MS") ?? 3_000;
        ...
        if (Environment.GetEnvironmentVariable("POCKETCALCULATOR_SHOT_ANIMATION_TIME_MS") is { } raw)
        {
            if (!float.TryParse(raw, NumberStyles.Float, CultureInfo.InvariantCulture, out var ms)
                || !float.IsFinite(ms) || ms < 0.0f)
            {
                throw new CliException(
                    "POCKETCALCULATOR_SHOT_ANIMATION_TIME_MS must be a finite non-negative number");
            }
        ...
        // CLI screenshots.
        var warmupCapture = Environment.GetEnvironmentVariable("POCKETCALCULATOR_SHOT_RESOURCE_WARMUP") == "1";
        if (warmupCapture)
        {
            if (Capture() is null)
            {
    ...
    {
        if (Environment.GetEnvironmentVariable("POCKETCALCULATOR_SHOT_SCROLL_Y") is not { } rawY)
        {
            return null;
        }
        double x = 0.0;
        if (Environment.GetEnvironmentVariable("POCKETCALCULATOR_SHOT_SCROLL_X") is { } rawX)
        {
        ...
        bool stealth)
    {
        ...
        using var client = new PocketCalculatorHttpClient(new CookieJar(), proxy);
        if (userAgent is { } ua)
        {
        ...
        catch (PocketCalculatorNetException error)
        {
        ...
        bool stealth)
    {
        ...
        for (var i = 0; i < total; i++)
        {
            ...
            tasks[i] = Task.Run(async () =>
            {
                ...
                catch (Exception error) when (error is CliException or PocketCalculatorNetException)
                {
                    return new JsonObject
                    {
                        ["url"] = JsonValue.Create(url),
