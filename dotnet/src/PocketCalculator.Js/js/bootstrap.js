...
// browser APIs below close over this reference. (In the C# port the bridge is
...
// `take_ops_handoff` / `share_ops_with_realm`). The C# host already holds the
...
globalThis.getComputedStyle = (el, pseudoElt) => {
  ...
  const refreshRendered = () => {
    ...
    if (el?._nid != null) {
      ...
      const op = pseudo
        ? __obscuraCore.ops.op_computed_style_pseudo
        : __obscuraCore.ops.op_computed_style;
      if (typeof op === 'function') {
        try {
          const raw = pseudo
...
// call (so an observer watching two nodes sees every mutation twice), leaves
// the record queue alone on disconnect() (so a mutation recorded this turn is
// still delivered afterwards), ignores attributeFilter, and queues one promise
// job per mutation instead of one per checkpoint. Chromium does none of those,
...
function _queueMutationObserverMicrotask() {
  ...
  Promise.resolve().then(_notifyMutationObservers);
}

function _notifyMutationObservers() {
  _mutationObserverMicrotaskQueued = false;
  const notify = [..._mutationNotifySet];
  _mutationNotifySet.clear();
  for (const observer of notify) {
    const batch = observer._records.splice(0);
...
globalThis.MutationObserver = class MutationObserver {
  ...
  observe(target, options) {
    // observe() steps 3-4: attributeOldValue / attributeFilter imply
    // attributes, characterDataOldValue implies characterData. Without this an
    // observer registered as { attributeFilter: [...] } saw nothing at all.
    ...
    if (normalized.characterData === undefined && normalized.characterDataOldValue !== undefined) {
      normalized.characterData = true;
    }
    // A registered observer is listed once however many nodes it observes.
    // Pushing per observe() made __notifyMutation deliver one record per
    // registration, so an observer watching two nodes saw every mutation twice.
  ...
  disconnect() {
    this._targets = [];
    // disconnect() also empties the record queue, so a mutation already
...
globalThis.__notifyMutation = function(type, target_nid, addedNodes, removedNodes, attributeName, oldValue) {
  ...
  for (const obs of globalThis.__mutationObservers) {
    ...
    for (const t of obs._targets) {
      ...
      // attributeFilter names the only attributes this registration wants.
      if (type === 'attributes' && Array.isArray(t.options.attributeFilter)
...
function _pullResourceTimings() {
  if (!__obscuraCore.ops
    || typeof __obscuraCore.ops.op_resource_timings !== "function") return;
  var records;
  try {
...
if (typeof FontFace === 'undefined') {
  ...
  const _fontSourceLoaded = source => {
    if (!__obscuraCore.ops
      || typeof __obscuraCore.ops.op_font_resource_loaded !== 'function') return false;
    ...
    while ((match = pattern.exec(source))) {
      ...
      try { if (__obscuraCore.ops.op_font_resource_loaded(absolute)) return true; } catch (e) {}
    }
    return false;
  };
