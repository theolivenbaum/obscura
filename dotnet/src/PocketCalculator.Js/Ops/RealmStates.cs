...
{
    private readonly List<(object Context, uint FrameId, PocketCalculatorState State)> _entries = [];

    /// <summary>The realm whose script is running, when the runtime has named one.</summary>
    public PocketCalculatorState? Current { get; set; }

    public bool IsEmpty => _entries.Count == 0;

    public void Register(object context, uint frameId, PocketCalculatorState state)
    {
    ...
    public PocketCalculatorState? ByFrameId(uint frameId)
    {
    ...
    public PocketCalculatorState? ByContext(object context)
    {
        foreach (var entry in _entries)
        {
