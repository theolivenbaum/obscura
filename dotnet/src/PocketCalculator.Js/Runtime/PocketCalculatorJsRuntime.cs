...
public sealed partial class PocketCalculatorJsRuntime
    : IDisposable, PocketCalculator.Js.Ops.IPostedTaskSpawner, PocketCalculator.Js.Ops.IAsyncOpTracker
{
    ...
    private PocketCalculatorJsRuntime(string baseUrl, string? proxyUrl)
    {
        ...
        _standaloneModuleClient = new PocketCalculatorHttpClient(new CookieJar(), proxyUrl);
    ...
    public PocketCalculatorJsRuntime() : this("about:blank", null) { }

    /// <summary>A runtime whose documents resolve against <paramref name="baseUrl"/>.</summary>
    public static PocketCalculatorJsRuntime WithBaseUrl(string baseUrl) => new(baseUrl, null);
    ...
    /// </summary>
    public static PocketCalculatorJsRuntime WithBaseUrlAndProxy(string baseUrl, string? proxyUrl) =>
    ...
    /// from the op body: see <see cref="PocketCalculator.Js.Ops.AsyncOpBinding"/> for why the
    /// host cannot see the moment that matters.
    /// </summary>
    void PocketCalculator.Js.Ops.IAsyncOpTracker.OpStarted() => Interlocked.Increment(ref _pendingAsyncOps);
    ...
    /// </remarks>
    private sealed class RecordingModuleLoader(PocketCalculatorModuleLoader inner, PocketCalculatorJsRuntime owner)
        : DocumentLoader
    {
