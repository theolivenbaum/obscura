...
/// </summary>
/// <remarks>
/// The Rust loader holds a <c>Weak&lt;RefCell&lt;PocketCalculatorState&gt;&gt;</c> and reaches
/// into it per load. Keeping that as a provider delegate keeps the seam with the
/// op layer narrow: the loader never names <c>PocketCalculatorState</c>, and the runtime
...
public readonly record struct ModuleNetworkContext
{
    ...
    public PocketCalculatorHttpClient? Client { get; private init; }
...
/// </para>
/// <para>
/// Module fetches go through <see cref="PocketCalculatorHttpClient"/> so a module graph uses
/// the same cookie jar, identity, redirect and security policy, SSRF gate,
/// interception and callbacks as the entry module.
/// </para>
/// </remarks>
public sealed class PocketCalculatorModuleLoader : DocumentLoader, IDisposable
{
    ...
    public PocketCalculatorModuleLoader(string baseUrl)
        : this(baseUrl, null)
    {
    }
    ...
    /// </summary>
    public PocketCalculatorModuleLoader(string baseUrl, string? proxyUrl)
        : this(baseUrl, proxyUrl, new ImportMap())
    {
        _standaloneClient = new PocketCalculatorHttpClient(new CookieJar(), proxyUrl);
    }
    ...
    /// </summary>
    public PocketCalculatorModuleLoader(
        string baseUrl,
        string? proxyUrl,
        ImportMap importMap,
    ...
    private PocketCalculatorModuleLoader(string baseUrl, string? proxyUrl, ImportMap importMap)
    {
    ...
    private sealed class StaticGraphScope(PocketCalculatorModuleLoader owner) : IDisposable
    {
        private PocketCalculatorModuleLoader? _owner = owner;
