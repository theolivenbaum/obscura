...
/// <summary>
/// The version <c>initialize</c> reports in <c>serverInfo.version</c>.
/// </summary>
/// <remarks>
/// Port of <c>crates/obscura-mcp/build.rs</c>, which is the same resolution the
/// CLI's <c>--version</c> uses so MCP clients see the version the binary was
/// actually cut from: an explicit <c>POCKETCALCULATOR_VERSION</c> wins, then a GitHub tag
/// ref, then the package version with the short commit appended. Local builds are
/// not cut from a tag, so the bare workspace version cannot tell two checkouts
/// apart; tarball and minimal-image builds without git keep the bare version.
/// </remarks>
public static class BuildVersion
{
    ...
    private static string Resolve() => Explicit() ?? GitHubTag() ?? Local();

    private static string? Explicit() => Normalize(EnvValue("POCKETCALCULATOR_VERSION"));
    ...
    private static string Local()
    {
        var package = typeof(BuildVersion).Assembly
            ...
            ?? "0.0.0";
        // Strip any SourceLink metadata the SDK appends, so the shape matches the
        // Rust build's "<package>-dev+<sha>".
