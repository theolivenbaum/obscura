...
using EmulationDomain = PocketCalculator.Cdp.Domains.Emulation;
...
public sealed class PageDomainTests
{
    ...
    {
        Assert.Throws<PocketCalculator.Cdp.Domains.DomainError>(() =>
    ...
    public async Task CaptureScreenshotPreservesDefaultPngAndHonorsClipScale()
    {
        ...
        byte[] native;
        {
            PocketCalculator.Browser.Page page = ctx.GetSessionPage(session)!;
    ...
    public async Task NavigationEmitsTargetInfoChangedWithUrlAndTitle()
    {
        ...
        PocketCalculator.Browser.Page page = ctx.GetPage(pageId)!;
