...
{
    private RuntimeFixture(PocketCalculatorJsRuntime runtime) => Runtime = runtime;

    public PocketCalculatorJsRuntime Runtime { get; }
    ...
    public static RuntimeFixture Blank() => new(new PocketCalculatorJsRuntime());
...
public sealed class RuntimeTests
{
    ...
    private sealed class DelayedFetch : IDisposable
    {
        ...
        public PocketCalculatorJsRuntime Runtime => _fixture.Runtime;
    ...
    {
        // SynchronousTaskFloorMs in PocketCalculatorJsRuntime.
    ...
    public async Task QuiescentEventLoopRetainsDelayedNetworkAndDomUpdate()
    {
        using var fixture = RuntimeFixture.Setup("<html><body></body></html>");
        var rt = fixture.Runtime;
        using var client = new PocketCalculatorHttpClient(new CookieJar(), null, allowPrivateNetwork: true);
    ...
    [Fact(Skip = "this Rust test is #[cfg(not(feature = \"render\"))]: it asserts the synthetic non-render scroll compatibility state that bootstrap.js falls back to when op_scroll_offset / op_scroll_to are absent. PocketCalculator.Js has no render feature split - the render ops are always bound - so window.scrollTo clamps against real geometry here. The render counterpart, RenderedWindowScrollClampsAndGeometryIsViewportRelative, covers this build")]
    ...
    {
        using var owner = new CaptureRuntime(new PocketCalculatorJsRuntime());
    ...
    public void FractionalScrollRangesQuantizeGeometryAndPixelsAtOneX()
    {
        ...
        using var rootOwner = new CaptureRuntime(new PocketCalculatorJsRuntime());
    ...
    [Fact(Skip = "this Rust test is #[cfg(not(feature = \"render\"))]: it asserts bootstrap.js's synthetic 100x20 CSSOM fallback, used only when op_element_box is absent. PocketCalculator.Js has no render feature split - the render ops are always bound - so getBoundingClientRect returns real layout geometry. RenderedCssomRectsDistinguishNoBoxFromZeroSizeBox covers this build")]
    ...
    public void ABaseInsertedAfterTheFirstLayoutStillRetargetsResourceUrls()
    {
        ...
        rt.State.RenderResources = PocketCalculator.Render.RenderResourceCache.WithLoader(url =>
        {
            requested.Add(url);
    ...
    public async Task MissingRenderResourcePreservesPreparedLayoutAndScroll()
    {
        ...
        Assert.Same(prepared, rt.State.PreparedRender);
        Assert.Equal<PocketCalculator.Render.RetainedStyleMutation>(
            [PocketCalculator.Render.RetainedStyleMutation.Resource.Instance],
    ...
    public void ImageResourceArrivalRetainsStylesAndRebuildsIntrinsicGeometry()
    {
        ...
        rt.SetUrl("http://example.test/page");
        rt.SetViewport(80.0, 60.0);
        rt.State.RenderResources = PocketCalculator.Render.RenderResourceCache.WithLoader(_ => null);
        rt.RunPageInit();
        ...
        rt.SeedRenderImageResource(
            ...
            PocketCalculator.Render.ImageRequestProfile.NoCorsInclude,
            png);
        rt.SeedRenderImageResource(
            "http://example.test/late.png",
            PocketCalculator.Render.ImageRequestProfile.NoCorsInclude,
        ...
        Assert.Equal(
            1,
            rt.State.PendingStyleMutations.Count(m => m is PocketCalculator.Render.RetainedStyleMutation.Resource));
        Assert.Contains(
            rt.State.PendingStyleMutations,
            m => m is PocketCalculator.Render.RetainedStyleMutation.Attribute);
    ...
    public void DocumentRegionCapturePreservesLiveRuntimeStateAndResourceCache()
    {
        ...
        var (regionPng, regionError) = rt.ScreenshotPreparedRegion(
            PocketCalculator.Render.CaptureRegion.New(0.0f, 115.0f, 80.0f, 40.0f, 1.5f));
        Assert.Null(regionError);
        Assert.NotNull(regionPng);
        var (fullPng, fullError) = rt.ScreenshotPreparedRegion(
            PocketCalculator.Render.CaptureRegion.New(0.0f, 0.0f, 80.0f, fullHeight, 1.0f));
    ...
    {
        List<string> loads = [];
        byte[] font = PocketCalculator.Render.FontAssets.Load("liberation-serif");
    ...
    public void PreparedRenderSurvivesDetachedNoOpAndSameViewportUpdates()
    {
        ...
        var attribute = Assert.IsType<PocketCalculator.Render.RetainedStyleMutation.Attribute>(mutation);
    ...
    public void ForwardAnimationSamplesRetainStaticPreparedRender()
    {
        ...
        Assert.True(rt.SetAnimationSampleTime(new PocketCalculator.Render.AnimationSampleTime(100.0f)));
        var first = rt.ScreenshotPrepared((80.0f, 60.0f), "http://example.test/page");
        Assert.NotNull(first);
        var prepared = rt.State.PreparedRender;
        Assert.NotNull(prepared);

        Assert.True(rt.SetAnimationSampleTime(new PocketCalculator.Render.AnimationSampleTime(250.0f)));
    ...
    public void ForwardWaapiSampleUpdatesRetainedStyleAndPaint()
    {
        ...
        Assert.NotNull(rt.State.PreparedRender);
        Assert.Equal<PocketCalculator.Render.RetainedStyleMutation>(
            [new PocketCalculator.Render.RetainedStyleMutation.WaapiAnimation(boxNode!.Value)],
            rt.State.PendingStyleMutations);
    ...
    public void CompletedAnimationRetainsForwardFrameButBackwardSeekRebuilds()
    {
        ...
        Assert.True(rt.SetAnimationSampleTime(new PocketCalculator.Render.AnimationSampleTime(150.0f)));
        ...
        Assert.True(rt.SetAnimationSampleTime(new PocketCalculator.Render.AnimationSampleTime(300.0f)));
        var later = rt.ScreenshotPrepared((80.0f, 60.0f), "http://example.test/page");
        Assert.NotNull(later);
        Assert.True(completed!.AsSpan().SequenceEqual(later));
        Assert.Same(prepared, rt.State.PreparedRender);

        Assert.True(rt.SetAnimationSampleTime(new PocketCalculator.Render.AnimationSampleTime(0.0f)));
    ...
    public void UnsupportedCustomPropertyAnimationDoesNotKeepRenderDamageActive()
    {
        ...
        var prepared = rt.State.PreparedRender;
        Assert.NotNull(prepared);
        Assert.True(rt.SetAnimationSampleTime(new PocketCalculator.Render.AnimationSampleTime(5_000.0f)));
        var later = rt.ScreenshotPrepared((80.0f, 60.0f), "http://example.test/page");
        Assert.NotNull(later);
    ...
    public void RemoveAndReappendRestartsAnimationWithoutIntermediateFlush()
    {
        ...
        Assert.True(rt.SetAnimationSample(PocketCalculator.Render.AnimationSample.Document(1_100.0f)));
        Assert.NotNull(rt.ScreenshotPrepared((200.0f, 80.0f), "http://example.test/page"));
    ...
    public void ScopedAnimationEpochsSurviveLaterUnrelatedMutationsAndT0Capture()
    {
        ...
        Assert.True(rt.SetAnimationSample(PocketCalculator.Render.AnimationSample.LocalOverride(0.0f)));
        Assert.NotNull(rt.ScreenshotPrepared((200.0f, 80.0f), "http://example.test/page"));
        ...
        Assert.True(rt.SetAnimationSample(PocketCalculator.Render.AnimationSample.Document(700.0f)));
        Assert.NotNull(rt.ScreenshotPrepared((200.0f, 80.0f), "http://example.test/page"));
    ...
    public void FixedAnimationCaptureIsInvariantAfterGeometryFlush()
    {
        ...
        var fixedSample = PocketCalculator.Render.AnimationSample.LocalOverride(750.0f);
    ...
    public void ConnectedTreeMutationsQueueRetainedStylesUntilGeometryFlush()
    {
        ...
        Assert.NotNull(rt.State.PreparedRender);
        var insert = Assert.IsType<PocketCalculator.Render.RetainedStyleMutation.Tree>(
            Assert.Single(rt.State.PendingStyleMutations));
        Assert.IsType<PocketCalculator.Render.TreeStyleMutation.Insert>(insert.Mutation);
        Assert.Equal(
            80.0,
        ...
        Assert.NotNull(rt.State.PreparedRender);
        var styled = Assert.IsType<PocketCalculator.Render.RetainedStyleMutation.Attribute>(
            Assert.Single(rt.State.PendingStyleMutations));
        ...
        Assert.NotNull(rt.State.PreparedRender);
        var removed = Assert.IsType<PocketCalculator.Render.RetainedStyleMutation.Tree>(
            Assert.Single(rt.State.PendingStyleMutations));
        Assert.IsType<PocketCalculator.Render.TreeStyleMutation.Remove>(removed.Mutation);
        Assert.Equal(
            40.0,
            rt.Evaluate("document.getElementById('added').getBoundingClientRect().width")!.GetValue<double>());
        Assert.Empty(rt.State.PendingStyleMutations);
    ...
    public async Task IntersectionObserverHonorsRootMarginZeroAreaAndNoFakeRefires()
    {
        ...
        /*
            async fn intersection_observer_honors_root_margin_zero_area_and_no_fake_refires() {
                ...
                let mut rt = PocketCalculatorJsRuntime::new();
                rt.set_dom(dom);
    ...
    public void Canvas2dLiveBackingPaintsImmediatelyWithScalingClipsAndEffects()
    {
        ...
        Assert.True(PocketCalculator.Js.Ops.RenderState.EnsureResolvedScroll(rt.State));
        var prepared = rt.State.PreparedRender;
        Assert.NotNull(prepared);
        ...
        Assert.True(PocketCalculator.Js.Ops.RenderState.EnsureResolvedScroll(rt.State));
        using var pixmap = PocketCalculator.Render.RenderPaint.PaintPreparedWithScrollAndSurfaceColorAndCanvasSurfaces(
            rt.State.Dom!,
            rt.State.PreparedRender!,
            rt.State.RenderResources,
            rt.State.ResolvedScroll!.Value.State,
            new PocketCalculator.Render.Css.RgbaColor(255, 255, 255, 255),
    ...
    public async Task ParserImagesLoadConcurrentlyWithoutBlockingTheEventLoop()
    {
        ...
        /*
            async fn parser_images_load_concurrently_without_blocking_the_event_loop() {
                ...
                rt.set_http_client(std::sync::Arc::new(
                    obscura_net::PocketCalculatorHttpClient::with_full_options(
        ...
        */
        ...
        rt.SetHttpClient(new PocketCalculatorHttpClient(new CookieJar(), null, allowPrivateNetwork: true));
        rt.RunPageInit();
    ...
    /// </remarks>
    private static async Task PumpUntilProfileEventsAsync(PocketCalculatorJsRuntime rt, int count)
    {
    ...
    public async Task ParserImageFallbackInvalidatesOnlyNewIntrinsicGeometry()
    {
        ...
        Assert.NotNull(PocketCalculator.Js.Ops.RenderState.EnsurePreparedRender(missing.State));
    ...
    public void TestSequentialRuntimeSwap()
    {
        ...
        Assert.NotNull(dom1);
        using var restored = new PocketCalculatorJsRuntime();
    ...
    public async Task EntryModuleHttpFailureIsNotEvaluatedAsEmptySource()
    {
        ...
        /*
            async fn entry_module_http_failure_is_not_evaluated_as_empty_source() {
                ...
                let client = std::sync::Arc::new(obscura_net::PocketCalculatorHttpClient::with_full_options(
                    jar, None, true,
                ));
                let mut rt = PocketCalculatorJsRuntime::with_base_url(&format!("{}/", base));
    ...
    public async Task DescendantModuleUsesPageCookieIdentityAndHeaders()
    {
        ...
        */
        ...
        var client = new PocketCalculatorHttpClient(jar, null, allowPrivateNetwork: true);
        ...
        using var rt = PocketCalculatorJsRuntime.WithBaseUrl($"{server.Origin}/");
        rt.SetCookieJar(jar);
        rt.SetHttpClient(client);
    ...
    public async Task CrossOriginModuleDescendantDoesNotGainModuleOriginCookies()
    {
        ...
        /*
            async fn cross_origin_module_descendant_does_not_gain_module_origin_cookies() {
                ...
                let mut rt = PocketCalculatorJsRuntime::with_base_url(document_url);
                rt.set_http_client(client);
    ...
    public async Task ImportMapResolvesPrefixStaticAndExactDynamicImports()
    {
        ...
        /*
            async fn import_map_resolves_prefix_static_and_exact_dynamic_imports() {
                ...
                let mut rt = PocketCalculatorJsRuntime::with_base_url(&format!("{}/app/index.html", base));
                rt.set_http_client(client);
                rt.add_import_map(
                    r#"{
                        "imports": {
    ...
    public async Task ImportMapDoesNotRemapExternalRootModuleUrl()
    {
        ...
        /*
            async fn import_map_does_not_remap_external_root_module_url() {
                ...
                let mut rt = PocketCalculatorJsRuntime::with_base_url(&format!("{}/index.html", base));
                rt.set_http_client(client);
                rt.add_import_map(
    ...
    public async Task InlineModulesExposeDocumentBaseAsImportMetaUrl()
    {
        ...
        /*
            async fn inline_modules_expose_document_base_as_import_meta_url() {
                let mut rt = PocketCalculatorJsRuntime::with_base_url("https://example.com/page/index.html");
                rt.load_inline_module(
        ...
        */
        using var rt = PocketCalculatorJsRuntime.WithBaseUrl("https://example.com/page/index.html");
        await rt.LoadInlineModuleAsync(
            "globalThis.__first_inline_url = import.meta.url;",
            "https://example.com/base/",
            1_000);
    ...
    public async Task ClassicScriptUrlIsDynamicImportReferrer()
    {
        ...
        /*
            async fn classic_script_url_is_dynamic_import_referrer() {
                ...
                let mut rt = PocketCalculatorJsRuntime::with_base_url(&format!("{base}/page/index.html"));
                rt.set_http_client(client);
    ...
    public async Task InlineModuleGraphErrorPropagates()
    {
        ...
        /*
            async fn inline_module_graph_error_propagates() {
                let mut rt = PocketCalculatorJsRuntime::with_base_url("https://example.com/");
                let error = rt
    ...
    {
        var jar = new CookieJar();
        var configured = new PocketCalculatorHttpClient(jar, "http://proxy.test:8080");
        Assert.Equal("http://proxy.test:8080", configured.ProxyUrl);

        var direct = new PocketCalculatorHttpClient(jar, null);
    ...
    {
        var loader = new PocketCalculatorModuleLoader("https://example.com/", "http://proxy.test:8080");
        ...
        var direct = new PocketCalculatorModuleLoader("https://example.com/", null);
        Assert.Null(direct.ProxyUrl);
    }
    ...
    public void RuntimeWithBaseUrlAndProxyConstructsSuccessfully()
    {
        ...
        using var direct = PocketCalculatorJsRuntime.WithBaseUrlAndProxy("https://example.com/", null);
        using var proxied = PocketCalculatorJsRuntime.WithBaseUrlAndProxy("https://example.com/", "http://proxy.test:8080");
        Assert.Null(direct.ProxyUrl);
    ...
    public void PostMessageHonoursTargetOrigin()
    {
        ...
        /*
            fn post_message_honours_target_origin() {
                ...
                let deliver = |parent: &mut PocketCalculatorJsRuntime, m: &crate::ops::PendingFrameMessage| {
                    let script = format!(
                        "globalThis.__obscura_deliverMessage({}, {}, {}, {});",
    ...
    public void AFloodOfMessagesCannotGrowTheQueueWithoutBound()
    {
        ...
        /*
            fn a_flood_of_messages_cannot_grow_the_queue_without_bound() {
                std::env::set_var("POCKETCALCULATOR_FRAME_MESSAGE_QUEUE_ENTRIES", "64");
                let mut parent = page("https://parent.example/", "<html><body></body></html>");
                let frame = FrameRealm::new(
                    &mut parent,
                    1,
                ...
                std::env::remove_var("POCKETCALCULATOR_FRAME_MESSAGE_QUEUE_ENTRIES");
            }
        */
        var previous = Environment.GetEnvironmentVariable("POCKETCALCULATOR_FRAME_MESSAGE_QUEUE_ENTRIES");
        Environment.SetEnvironmentVariable("POCKETCALCULATOR_FRAME_MESSAGE_QUEUE_ENTRIES", "64");
        try
        {
            using var page = RuntimeFixture.Page("https://parent.example/", "<html><body></body></html>");
            var parent = page.Runtime;
        ...
        {
            Environment.SetEnvironmentVariable("POCKETCALCULATOR_FRAME_MESSAGE_QUEUE_ENTRIES", previous);
        }
    }
    ...
    private static RuntimeFixture ParserImageRuntime(string html, Func<string, byte[]?> loader)
    {
        ...
        runtime.State.RenderResources = PocketCalculator.Render.RenderResourceCache.WithLoader(loader);
        runtime.RunPageInit();
        return fixture;
    }
    ...
    /// </summary>
    private static PocketCalculatorJsRuntime ModuleGraphRuntime(string baseUrl, CookieJar jar)
    {
        var runtime = PocketCalculatorJsRuntime.WithBaseUrl(baseUrl);
