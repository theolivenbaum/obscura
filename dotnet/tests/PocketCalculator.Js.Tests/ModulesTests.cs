...
public sealed class ModuleLoaderTests
{
    ...
    {
        using var loader = new PocketCalculatorModuleLoader(Base);
    ...
    {
        var map = new ImportMap();
        using var loader = new PocketCalculatorModuleLoader(
            Base,
            null,
            map,
    ...
    {
        using var loader = new PocketCalculatorModuleLoader(Base, "http://proxy.test:8080");
    ...
    private static PocketCalculatorModuleLoader LoaderWith(string importMapJson)
    {
        Assert.True(ImportMap.TryParse(importMapJson, Base, out var map, out var error), error);
        return new PocketCalculatorModuleLoader(
            Base,
            null,
            map,
            () => ModuleNetworkContext.Failed("no network in this test"));
    ...
    private static string Resolve(PocketCalculatorModuleLoader loader, string specifier, string referrer)
    {
...
/// <see cref="PocketCalculator.Net.PocketCalculatorHttpClient"/> and evaluated by V8, which is the
...
public sealed class ModuleGraphLoadTests : IDisposable
{
    ...
    private readonly PocketCalculator.Net.PocketCalculatorHttpClient _client;
    ...
    public ModuleGraphLoadTests()
    {
        ...
        _client = new PocketCalculator.Net.PocketCalculatorHttpClient(
            new PocketCalculator.Net.CookieJar(),
            null,
    ...
    private static V8ScriptEngine NewEngine(PocketCalculatorModuleLoader loader)
    {
