...
public sealed class UnhandledRejectionTests
{
    ...
    // BLOCKED on a gap in PocketCalculator.Js, not on this port: `DenoCoreShim` stores the
    ...
    [Fact(Skip = "blocked on PocketCalculator.Js: the handler registered through Deno.core.setUnhandledPromiseRejectionHandler is stored by DenoCoreShim and never invoked, so no unhandledrejection event is dispatched")]
