...
public sealed class PageTests
{
    ...
    {
        string? previous = Environment.GetEnvironmentVariable("POCKETCALCULATOR_NAV_CHAIN_LIMIT");
        Environment.SetEnvironmentVariable("POCKETCALCULATOR_NAV_CHAIN_LIMIT", "17");
        try
        {
        ...
        {
            Environment.SetEnvironmentVariable("POCKETCALCULATOR_NAV_CHAIN_LIMIT", previous);
        }
    }
    ...
    // BLOCKED on a bug in PocketCalculator.Js, not on this port: `op_frame_document_ready`
    ...
    [Fact(Skip = "blocked on PocketCalculator.Js: op_frame_document_ready records parentFrameId 0 for a frame created inside an async continuation, because RealmStates.Current is only set around synchronous host entries")]
    ...
    // `PocketCalculatorState.ImportMap`. The two are different objects, so a map registered
    ...
    [Fact(Skip = "blocked on PocketCalculator.Js: PocketCalculatorJsRuntime builds the module loader with its own new ImportMap() while op_add_import_map writes into PocketCalculatorState.ImportMap, so a map registered from JavaScript never reaches module resolution. Rust shares one Rc<RefCell<ImportMap>> between the two.")]
    ...
    [Fact(Skip = "blocked on PocketCalculator.Js: ClearScript resolves a dynamic import() synchronously during script evaluation, so it cannot be post-load work as it is under deno_core")]
    ...
    {
        using var runtime = new PocketCalculatorJsRuntime();
