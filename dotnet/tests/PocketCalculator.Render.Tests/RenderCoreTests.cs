...
public class RenderCoreTests
{
    ...
    // (PocketCalculator.Render.Layout.TaffyTree) and, for the grid case, on style.rs `compute_style`.
    ...
    public void WaapiControlOperationsMoveTheLocalClock()
    {
        ...
        {
            Id = 7,
            Node = new PocketCalculator.Dom.NodeId(3),
        ...
        Assert.Equal(new PocketCalculator.Dom.NodeId(3), state.WaapiNode(7)!.Value);
        Assert.Contains(new PocketCalculator.Dom.NodeId(3), state.WaapiNodes());
    ...
    public void StartCandidatesGateTheStyleFlush()
    {
        ...
        state.NoteStartCandidate(new PocketCalculator.Dom.NodeId(1), 40f);
        Assert.True(state.HasPendingStartCandidates());
        // Non-finite and negative document times are ignored.
        state.ClearStartCandidates();
        state.NoteStartCandidate(new PocketCalculator.Dom.NodeId(1), -1f);
        state.NoteSubtreeStartCandidate(new PocketCalculator.Dom.NodeId(2), float.NaN);
        Assert.False(state.HasPendingStartCandidates());
    }
