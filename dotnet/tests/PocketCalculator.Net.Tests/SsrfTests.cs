...
public class SsrfTests
{
    ...
    private static void AssertBlocked(string url) =>
        Assert.Throws<PocketCalculatorNetException>(() => SsrfGuard.ValidateUrl(new Uri(url), false));
    ...
    public void SubresourceReferrerAndFetchSiteFollowDefaultBrowserPolicy()
    {
        ...
        Assert.Equal("same-origin", PocketCalculatorHttpClient.RequestFetchSite(request, sameOrigin));
        Assert.Equal("cross-site", PocketCalculatorHttpClient.RequestFetchSite(request, crossOrigin));
        Assert.Equal(
            "https://app.example/path?q=1",
            PocketCalculatorHttpClient.RequestReferrer(request, sameOrigin));
        Assert.Equal(
            "https://app.example/",
            PocketCalculatorHttpClient.RequestReferrer(request, crossOrigin));
    ...
    public async Task NavigationFollowsTwentyRedirectsButNotTwentyOne()
    {
        ...
        using var failClient = new PocketCalculatorHttpClient(new CookieJar(), null, true);
        var error = await Assert.ThrowsAsync<PocketCalculatorNetException>(
            () => failClient.FetchAsync(failFixture.Url));
        Assert.Equal(PocketCalculatorNetErrorKind.TooManyRedirects, error.Kind);
    }
    ...
    public async Task CrossOriginFontSendsOriginAndOmitsCrossOriginCookies()
    {
        ...
        using var client = new PocketCalculatorHttpClient(jar, null, true);
    ...
    // #849 - the POCKETCALCULATOR_FETCH_MAX_BODY_BYTES override #581 gave fetch()/XHR must
    ...
    public void ModuleScriptCapHonoursTheFetchBodyEnvOverride()
    {
        ...
        env.Set("POCKETCALCULATOR_FETCH_MAX_BODY_BYTES", null);
        Assert.Equal(32L * 1024 * 1024, ResourceRequest.ModuleScript(u, u).MaxResponseBytes);

        Environment.SetEnvironmentVariable("POCKETCALCULATOR_FETCH_MAX_BODY_BYTES", "134217728");
        ...
        Environment.SetEnvironmentVariable("POCKETCALCULATOR_FETCH_MAX_BODY_BYTES", "not-a-number");
        Assert.Equal(32L * 1024 * 1024, ResourceRequest.ModuleScript(u, u).MaxResponseBytes);
    }
    ...
    public async Task CredentialedCorsRejectsWildcardAndAcceptsExactOrigin()
    {
        ...
        Assert.Equal(PocketCalculatorNetErrorKind.Cors, error.Kind);
    ...
    public async Task CancellationReturnsActiveRequestsToZero()
    {
        ...
        catch (Exception error) when (error is OperationCanceledException or PocketCalculatorNetException)
        {
            // expected
        }
    ...
    public async Task TransportTimeoutReturnsActiveRequestsToZero()
    {
        ...
        Assert.Equal(PocketCalculatorNetErrorKind.Network, error.Kind);
        Assert.Equal(0, client.ActiveRequests);
    }
