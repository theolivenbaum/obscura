...
using Xunit;
// `PocketCalculator.Browser` is also a namespace, and namespace lookup in an enclosing
...
// `Browser` after `using PocketCalculator.Api;`.
using ApiBrowser = PocketCalculator.Api.Browser;

namespace PocketCalculator.Cli.Tests;

/// <summary>
/// Tests for the embeddable library API (<c>crates/obscura</c> -&gt; the
/// <c>PocketCalculator</c> project). The named integration-test files under
/// <c>crates/obscura/tests/</c> have their own counterparts beside this one.
/// </summary>
/// <remarks>
/// These belong in an <c>PocketCalculator.Tests</c> project. They live here because
...
/// slightly wrong home.
/// </remarks>
public sealed class PocketCalculatorApiTests
{
    ...
    public async Task Click_on_a_handle_from_a_previous_document_reports_element_not_found()
    {
        ...
        var error = Assert.Throws<PocketCalculatorException>(element!.Click);
        Assert.Equal(PocketCalculatorErrorKind.ElementNotFound, error.Kind);
    ...
    public async Task Wait_for_selector_does_not_pump_the_event_loop()
    {
        ...
        await Assert.ThrowsAsync<PocketCalculatorException>(
    ...
    public async Task Wait_for_selector_times_out_with_the_rust_message()
    {
        ...
        var error = await Assert.ThrowsAsync<PocketCalculatorException>(
            () => page.WaitForSelectorAsync("#never", TimeSpan.FromMilliseconds(200)));
        Assert.Equal(PocketCalculatorErrorKind.Timeout, error.Kind);
    ...
    public async Task Goto_reports_a_navigation_error_rather_than_a_page_exception()
    {
        ...
        Assert.Equal(PocketCalculatorErrorKind.Navigation, error.Kind);
    ...
    public void Cookie_store_rejects_a_relative_url_the_way_rust_does()
    {
        ...
        var error = Assert.Throws<PocketCalculatorException>(() => cookies.Set("a=b", "not-a-url"));
        Assert.Equal(PocketCalculatorErrorKind.Internal, error.Kind);
    }

    [Fact]
    public void Error_messages_match_the_thiserror_renderings()
    {
        Assert.Equal("navigation error: x", PocketCalculatorException.Navigation("x").Message);
        Assert.Equal("JS evaluation error: x", PocketCalculatorException.JsEval("x").Message);
        Assert.Equal("timeout: x", PocketCalculatorException.Timeout("x").Message);
        Assert.Equal("element not found: x", PocketCalculatorException.ElementNotFound("x").Message);
        Assert.Equal("no page session", PocketCalculatorException.NoPage().Message);
        Assert.Equal("boom", PocketCalculatorException.Internal(new InvalidOperationException("boom")).Message);
    }
}
