...
/// <c>PocketCalculator.Render.Tests/FontDirectoryTests.cs</c>.
/// </summary>
public sealed class FontDirTests
{
    ...
    {
        string missing = MissingDirectory();
        var error = Assert.Throws<PocketCalculatorException>(
            () => PocketCalculator.Api.Browser.Builder().FontDirectory(missing).Build());
        Assert.Equal($"Font directory does not exist or is not a directory: {missing}", error.Message);
    }
    ...
    private static string ArabicFixture([CallerFilePath] string path = "") =>
        Path.GetFullPath(Path.Combine(
            Path.GetDirectoryName(path)!, "..", "PocketCalculator.Render.Tests", "Fixtures", "fonts", "NotoSansArabic.ttf"));
