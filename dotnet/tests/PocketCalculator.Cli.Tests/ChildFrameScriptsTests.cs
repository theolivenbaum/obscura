...
public sealed class ChildFrameScriptsTests
{
    ...
    public async Task A_rejected_child_frame_does_not_leave_js_references()
    {
        ...
        var previous = Environment.GetEnvironmentVariable("POCKETCALCULATOR_MAX_LIVE_FRAMES");
        Environment.SetEnvironmentVariable("POCKETCALCULATOR_MAX_LIVE_FRAMES", "0");
        try
        {
            var browser = ApiBrowser.New();
            using var page = await browser.NewPageAsync();
        ...
        {
            Environment.SetEnvironmentVariable("POCKETCALCULATOR_MAX_LIVE_FRAMES", previous);
        }
    }
