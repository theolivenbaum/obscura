...
public sealed class LayoutDeadlineTests : IDisposable
{
    ...
    {
        // 300 nested floats: one getBoundingClientRect() takes well over a minute.
        var path = Path.Combine(_directory, "floats.html");
        File.WriteAllText(
            path,
            "<!doctype html><body><script>let p=document.body;for(let i=0;i<300;i++){"
            + "const d=document.createElement('div');d.style.cssText='float:left;padding:1px';"
            + "d.textContent='x';p.appendChild(d);p=d;}p.getBoundingClientRect();"
            + "document.title='finished';</script>");

        var clock = Stopwatch.StartNew();
        var run = CliProcess.Run(
            "fetch", new Uri(path).AbsoluteUri, "--timeout", "3", "--quiet", "--eval", "document.title");
